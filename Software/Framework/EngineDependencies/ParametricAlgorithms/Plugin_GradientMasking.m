function Plugin = Plugin_GradientMasking
% SUMMARY:
...
% thresholding of a gradient
    ...
    % Parameter 3
    Plugin.controls(3).Name   = 'Gradient Threshold';
    Plugin.controls(3).Symbol = '$\epsilon_g';
    Plugin.controls(3).Units  = 'gradient magnitude';
    ...
    Plugin.Layers(3).Name     = 'gradientmapmask';
    Plugin.Layers(3).In       = [0 0 1];
    Plugin.Layers(3).DataName = 'gradient-map-mask';
    Plugin.Layers(3).Process  = 'Thresholds Gradient Magnitude';
    Plugin.Layers(3).Forward  = @(d, p) SegmentByGradient(d{3}, p{3});
    % Convex hull
    ...
    Plugin.Layers(4).Forward  = @(d, p) CleanUpMask(d{4});
end


function Mask = CleanUpMask(Mask)
